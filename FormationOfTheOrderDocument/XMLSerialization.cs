    ...
    class XMLSerialization
    {
        ...
        public string GetXML(Models.Product[] products, Action GenerateXML)
        {
            ...
            for(int i =0; i<products.Length;i++)
            {
                ...
                stringBuilder.Append("<СведТов НомСтр=\"" + (i + 1) + "\" НаимТов=\"" + products[i].Name + "\" ОКЕИ_Тов=\"796\" КолТов=\"" + products[i].Count + "\" ЦенаТов=\"" + products[i].Price + "\" СтТовБезНДС=\"0\" НалСт=\"без НДС\" СтТовУчНал=\"0\"><Акциз><БезАкциз>без акциза</БезАкциз></Акциз><СумНал><БезНДС>без НДС</БезНДС></СумНал><ДопСведТов ПрТовРаб=\"1\" НаимЕдИзм=\"шт\" НадлОтп=\"" + products[i].Count + "\" КодТов=\"" + products[i].BarCode + "\"/></СведТов>");
            }
